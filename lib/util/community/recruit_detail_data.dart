List<Map<String, dynamic>> recruitDetailData = [
  {
    'id': '1',
    'companyName': '싸이몬',
    'location_Si_Gu': '서울시 강남구',
    'location_Gu': '강남구',
    'title': 'UX/GUI 디자이너 모집',
    'image': 'assets/images/community/recruit/recruit_01.jpg',
    'stack': [
      'HTML',
      'Figma',
      'Adobe XD',
      'C4D',
      'Adobe Photoshop',
    ],
    'due': '상시',
    'location_detail': '서울 서초구 효령료 72길 15 2층',
    'description_brief': {
      '경력': '경력',
      '교육': '대졸(2,3년제) 이상',
      '근무형태': '대졸(2,3년제) 이상',
      '급여': '면접 후 결정',
      '직급/직책': '면접 후 결정',
      '근무일시': '주 5일(월~금) 10:00~19:00',
    },
    'description':
        '''액티블리: 산책 기록, 친구 찾기는 현실 지도를 기반으로 산책/운동을 기록하며 주변의 친구를 만나는게임형 액티비티 소셜 플랫폼 입니다.

주요업무
- 게임형 액티비티 소셜 플랫폼 “액티블리”의 UI 디자인을 진행합니다.
- 기획자와 UX설계를 논의하며 UI 디자인을 합니다.
- 디자인 시스템을 구축하고 관리합니다.

자격요건
- 디자인 결과물을 뽑아내는 프로세스를 논리적인 사고 과정으로 풀어낼 수 있으신 분
- 팀의 목표를 위한 협업과 디자인의 독립적인 업무를 주도적으로 이끌어갈 수 있으신 분-
 영어로 리서치 가능하신 분
- 위치기반 서비스를 좋아하거나, 지대한 관심이 있거나, 앞으로라도 지대한 관심을 가져보실 분

우대사항
- 처음부터 서비스를 만들어 출시해 본 경험이 있으신 분
- 인터렉티브한 디자인 가능하신 분
- 3D 오브젝트 제작 가능하신 분
- 최신 트랜드에 대한 관심이 많으신 분

혜택 및 복지
라이프페스타는 가치 있는 일과 신뢰 하는 동료들과 함께 일과 삶의 조화를 찾아가고 있습니다.
• 업무 성과에 따른 스톡옵션 부여 
• 자율적인 연차 사용 
• 듀얼 모니터 등 직군에 맞는 업무 장비 지원 
• 성장을 위한 도서 구입비 지원 
• 쾌적하고 자유로운 "초"역세권 근무환경''',
    'companyInfo': {
      '기업형태': '중소기업',
      '업종': '에이전시',
      '매출액': '연 매출 2억',
      '사원수': '13명',
      '설립일': '2018년 1월 31일',
      '대표자': '김이나',
    },
    'stastics': {
      'applicant': {
        '지원자 수': 12,
        '모집인원': 0,
      },
      'document': {
        '포트폴리오': 10,
        '기타 문서': 2,
      },
      'career': {
        '신입': 6,
        '1년 미만': 3,
        '1~3년': 2,
        '3~5년': 0,
        '5년 이상': 1,
      },
    },
    'recommand': [
      {
        'id': '2',
        'companyName': '이노이',
        'title': 'UI 시각디자이너',
        'location_Gu': '강남구',
        'due': '01.19(금)',
        'image': 'assets/images/community/recruit/recruit_10.jpg',
      },
      {
        'id': '3',
        'companyName': '이그나이트',
        'title': '시각 디자이너 모집',
        'location_Gu': '강남구',
        'due': '01.19(금)',
        'image': 'assets/images/community/recruit/recruit_13.jpg',
      },
      {
        'id': '4',
        'companyName': '캐럿디자인',
        'title': '웹디자이너 신입',
        'location_Gu': '강남구',
        'due': '01.19(금)',
        'image': 'assets/images/community/recruit/recruit_12.jpg',
      },
      {
        'id': '5',
        'companyName': '솔유',
        'title': '프로덕트 디자이너(UX/UI)',
        'location_Gu': '강남구',
        'due': '01.19(금)',
        'image': 'assets/images/community/recruit/recruit_11.jpg',
      },
    ],
  },
  {
    'id': '2',
    'companyName': '이노이',
    'title': 'UI 시각디자이너',
    'due': '01.19(금)',
    'location_Si_Gu': '서울시 강남구',
    'location_Gu': '강남구',
    'image': 'assets/images/community/recruit/recruit_06.jpg',
    'stack': [
      'HTML',
      'Figma',
      'Adobe XD',
      'C4D',
      'Adobe Photoshop',
    ],
    'location_detail': '서울 서초구 효령료 72길 15 2층',
    'description_brief': {
      '경력': '경력',
      '교육': '대졸(2,3년제) 이상',
      '근무형태': '대졸(2,3년제) 이상',
      '급여': '면접 후 결정',
      '직급/직책': '면접 후 결정',
      '근무일시': '주 5일(월~금) 10:00~19:00',
    },
    'description':
        '''액티블리: 산책 기록, 친구 찾기는 현실 지도를 기반으로 산책/운동을 기록하며 주변의 친구를 만나는게임형 액티비티 소셜 플랫폼 입니다.

주요업무
- 게임형 액티비티 소셜 플랫폼 “액티블리”의 UI 디자인을 진행합니다.
- 기획자와 UX설계를 논의하며 UI 디자인을 합니다.
- 디자인 시스템을 구축하고 관리합니다.

자격요건
- 디자인 결과물을 뽑아내는 프로세스를 논리적인 사고 과정으로 풀어낼 수 있으신 분
- 팀의 목표를 위한 협업과 디자인의 독립적인 업무를 주도적으로 이끌어갈 수 있으신 분-
 영어로 리서치 가능하신 분
- 위치기반 서비스를 좋아하거나, 지대한 관심이 있거나, 앞으로라도 지대한 관심을 가져보실 분

우대사항
- 처음부터 서비스를 만들어 출시해 본 경험이 있으신 분
- 인터렉티브한 디자인 가능하신 분
- 3D 오브젝트 제작 가능하신 분
- 최신 트랜드에 대한 관심이 많으신 분

혜택 및 복지
라이프페스타는 가치 있는 일과 신뢰 하는 동료들과 함께 일과 삶의 조화를 찾아가고 있습니다.
• 업무 성과에 따른 스톡옵션 부여 
• 자율적인 연차 사용 
• 듀얼 모니터 등 직군에 맞는 업무 장비 지원 
• 성장을 위한 도서 구입비 지원 
• 쾌적하고 자유로운 "초"역세권 근무환경''',
    'companyInfo': {
      '기업형태': '중소기업',
      '업종': '에이전시',
      '매출액': '연 매출 2억',
      '사원수': '13명',
      '설립일': '2018년 1월 31일',
      '대표자': '김이나',
    },
    'stastics': {
      'applicant': {
        '지원자 수': 12,
        '모집인원': 0,
      },
      'document': {
        '포트폴리오': 10,
        '기타 문서': 2,
      },
      'career': {
        '신입': 6,
        '1년 미만': 3,
        '1~3년': 2,
        '3~5년': 0,
        '5년 이상': 1,
      },
    },
    'recommand': [
      {
        'id': '1',
        'companyName': '싸이몬',
        'title': 'UX/GUI 디자이너 모집',
        'location_Gu': '강남구',
        'due': '상시',
        'image': 'assets/images/community/recruit/recruit_01.jpg',
      },
      {
        'id': '3',
        'companyName': '이그나이트',
        'title': '시각 디자이너 모집',
        'location_Gu': '강남구',
        'due': '01.19(금)',
        'image': 'assets/images/community/recruit/recruit_13.jpg',
      },
      {
        'id': '4',
        'companyName': '캐럿디자인',
        'title': '웹디자이너 신입',
        'location_Gu': '강남구',
        'due': '01.19(금)',
        'image': 'assets/images/community/recruit/recruit_12.jpg',
      },
      {
        'id': '5',
        'companyName': '솔유',
        'title': '프로덕트 디자이너(UX/UI)',
        'location_Gu': '강남구',
        'due': '01.19(금)',
        'image': 'assets/images/community/recruit/recruit_11.jpg',
      },
    ],
  },
  {
    'id': '3',
    'companyName': '이그나이트',
    'title': '시각 디자이너 모집',
    'due': '01.19(금)',
    'location_Si_Gu': '서울시 강남구',
    'location_Gu': '강남구',
    'image': 'assets/images/community/recruit/recruit_03.jpg',
    'stack': [
      'HTML',
      'Figma',
      'Adobe XD',
      'C4D',
      'Adobe Photoshop',
    ],
    'location_detail': '서울 서초구 효령료 72길 15 2층',
    'description_brief': {
      '경력': '경력',
      '교육': '대졸(2,3년제) 이상',
      '근무형태': '대졸(2,3년제) 이상',
      '급여': '면접 후 결정',
      '직급/직책': '면접 후 결정',
      '근무일시': '주 5일(월~금) 10:00~19:00',
    },
    'description':
        '''액티블리: 산책 기록, 친구 찾기는 현실 지도를 기반으로 산책/운동을 기록하며 주변의 친구를 만나는게임형 액티비티 소셜 플랫폼 입니다.

주요업무
- 게임형 액티비티 소셜 플랫폼 “액티블리”의 UI 디자인을 진행합니다.
- 기획자와 UX설계를 논의하며 UI 디자인을 합니다.
- 디자인 시스템을 구축하고 관리합니다.

자격요건
- 디자인 결과물을 뽑아내는 프로세스를 논리적인 사고 과정으로 풀어낼 수 있으신 분
- 팀의 목표를 위한 협업과 디자인의 독립적인 업무를 주도적으로 이끌어갈 수 있으신 분-
 영어로 리서치 가능하신 분
- 위치기반 서비스를 좋아하거나, 지대한 관심이 있거나, 앞으로라도 지대한 관심을 가져보실 분

우대사항
- 처음부터 서비스를 만들어 출시해 본 경험이 있으신 분
- 인터렉티브한 디자인 가능하신 분
- 3D 오브젝트 제작 가능하신 분
- 최신 트랜드에 대한 관심이 많으신 분

혜택 및 복지
라이프페스타는 가치 있는 일과 신뢰 하는 동료들과 함께 일과 삶의 조화를 찾아가고 있습니다.
• 업무 성과에 따른 스톡옵션 부여 
• 자율적인 연차 사용 
• 듀얼 모니터 등 직군에 맞는 업무 장비 지원 
• 성장을 위한 도서 구입비 지원 
• 쾌적하고 자유로운 "초"역세권 근무환경''',
    'companyInfo': {
      '기업형태': '중소기업',
      '업종': '에이전시',
      '매출액': '연 매출 2억',
      '사원수': '13명',
      '설립일': '2018년 1월 31일',
      '대표자': '김이나',
    },
    'stastics': {
      'applicant': {
        '지원자 수': 12,
        '모집인원': 0,
      },
      'document': {
        '포트폴리오': 10,
        '기타 문서': 2,
      },
      'career': {
        '신입': 6,
        '1년 미만': 3,
        '1~3년': 2,
        '3~5년': 0,
        '5년 이상': 1,
      },
    },
    'recommand': [
      {
        'id': '1',
        'companyName': '싸이몬',
        'title': 'UX/GUI 디자이너 모집',
        'location_Gu': '강남구',
        'due': '상시',
        'image': 'assets/images/community/recruit/recruit_01.jpg',
      },
      {
        'id': '2',
        'companyName': '이노이',
        'title': 'UI 시각디자이너',
        'location_Gu': '강남구',
        'due': '01.19(금)',
        'image': 'assets/images/community/recruit/recruit_10.jpg',
      },
      {
        'id': '4',
        'companyName': '캐럿디자인',
        'title': '웹디자이너 신입',
        'location_Gu': '강남구',
        'due': '01.19(금)',
        'image': 'assets/images/community/recruit/recruit_12.jpg',
      },
      {
        'id': '5',
        'companyName': '솔유',
        'title': '프로덕트 디자이너(UX/UI)',
        'location_Gu': '강남구',
        'due': '01.19(금)',
        'image': 'assets/images/community/recruit/recruit_11.jpg',
      },
    ],
  },
  {
    'id': '4',
    'companyName': '캐럿디자인',
    'title': '웹디자이너 신입',
    'due': '01.19(금)',
    'location_Si_Gu': '서울시 강남구',
    'location_Gu': '강남구',
    'image': 'assets/images/community/recruit/recruit_04.jpg',
    'stack': [
      'HTML',
      'Figma',
      'Adobe XD',
      'C4D',
      'Adobe Photoshop',
    ],
    'location_detail': '서울 서초구 효령료 72길 15 2층',
    'description_brief': {
      '경력': '경력',
      '교육': '대졸(2,3년제) 이상',
      '근무형태': '대졸(2,3년제) 이상',
      '급여': '면접 후 결정',
      '직급/직책': '면접 후 결정',
      '근무일시': '주 5일(월~금) 10:00~19:00',
    },
    'description':
        '''액티블리: 산책 기록, 친구 찾기는 현실 지도를 기반으로 산책/운동을 기록하며 주변의 친구를 만나는게임형 액티비티 소셜 플랫폼 입니다.

주요업무
- 게임형 액티비티 소셜 플랫폼 “액티블리”의 UI 디자인을 진행합니다.
- 기획자와 UX설계를 논의하며 UI 디자인을 합니다.
- 디자인 시스템을 구축하고 관리합니다.

자격요건
- 디자인 결과물을 뽑아내는 프로세스를 논리적인 사고 과정으로 풀어낼 수 있으신 분
- 팀의 목표를 위한 협업과 디자인의 독립적인 업무를 주도적으로 이끌어갈 수 있으신 분-
 영어로 리서치 가능하신 분
- 위치기반 서비스를 좋아하거나, 지대한 관심이 있거나, 앞으로라도 지대한 관심을 가져보실 분

우대사항
- 처음부터 서비스를 만들어 출시해 본 경험이 있으신 분
- 인터렉티브한 디자인 가능하신 분
- 3D 오브젝트 제작 가능하신 분
- 최신 트랜드에 대한 관심이 많으신 분

혜택 및 복지
라이프페스타는 가치 있는 일과 신뢰 하는 동료들과 함께 일과 삶의 조화를 찾아가고 있습니다.
• 업무 성과에 따른 스톡옵션 부여 
• 자율적인 연차 사용 
• 듀얼 모니터 등 직군에 맞는 업무 장비 지원 
• 성장을 위한 도서 구입비 지원 
• 쾌적하고 자유로운 "초"역세권 근무환경''',
    'companyInfo': {
      '기업형태': '중소기업',
      '업종': '에이전시',
      '매출액': '연 매출 2억',
      '사원수': '13명',
      '설립일': '2018년 1월 31일',
      '대표자': '김이나',
    },
    'stastics': {
      'applicant': {
        '지원자 수': 12,
        '모집인원': 0,
      },
      'document': {
        '포트폴리오': 10,
        '기타 문서': 2,
      },
      'career': {
        '신입': 6,
        '1년 미만': 3,
        '1~3년': 2,
        '3~5년': 0,
        '5년 이상': 1,
      },
    },
    'recommand': [
      {
        'id': '1',
        'companyName': '싸이몬',
        'title': 'UX/GUI 디자이너 모집',
        'location_Gu': '강남구',
        'due': '상시',
        'image': 'assets/images/community/recruit/recruit_01.jpg',
      },
      {
        'id': '2',
        'companyName': '이노이',
        'title': 'UI 시각디자이너',
        'location_Gu': '강남구',
        'due': '01.19(금)',
        'image': 'assets/images/community/recruit/recruit_13.jpg',
      },
      {
        'id': '3',
        'companyName': '이그나이트',
        'title': '시각 디자이너 모집',
        'location_Gu': '강남구',
        'due': '01.19(금)',
        'image': 'assets/images/community/recruit/recruit_12.jpg',
      },
      {
        'id': '5',
        'companyName': '솔유',
        'title': '프로덕트 디자이너(UX/UI)',
        'location_Gu': '강남구',
        'due': '01.19(금)',
        'image': 'assets/images/community/recruit/recruit_11.jpg',
      },
    ],
  },
  {
    'id': '5',
    'companyName': '솔유',
    'title': '프로덕트 디자이너(UX/UI)',
    'due': '01.19(금)',
    'location_Si_Gu': '서울시 강남구',
    'location_Gu': '강남구',
    'image': 'assets/images/community/recruit/recruit_05.jpg',
    'stack': [
      'HTML',
      'Figma',
      'Adobe XD',
      'C4D',
      'Adobe Photoshop',
    ],
    'location_detail': '서울 서초구 효령료 72길 15 2층',
    'description_brief': {
      '경력': '경력',
      '교육': '대졸(2,3년제) 이상',
      '근무형태': '대졸(2,3년제) 이상',
      '급여': '면접 후 결정',
      '직급/직책': '면접 후 결정',
      '근무일시': '주 5일(월~금) 10:00~19:00',
    },
    'description':
        '''액티블리: 산책 기록, 친구 찾기는 현실 지도를 기반으로 산책/운동을 기록하며 주변의 친구를 만나는게임형 액티비티 소셜 플랫폼 입니다.

주요업무
- 게임형 액티비티 소셜 플랫폼 “액티블리”의 UI 디자인을 진행합니다.
- 기획자와 UX설계를 논의하며 UI 디자인을 합니다.
- 디자인 시스템을 구축하고 관리합니다.

자격요건
- 디자인 결과물을 뽑아내는 프로세스를 논리적인 사고 과정으로 풀어낼 수 있으신 분
- 팀의 목표를 위한 협업과 디자인의 독립적인 업무를 주도적으로 이끌어갈 수 있으신 분-
 영어로 리서치 가능하신 분
- 위치기반 서비스를 좋아하거나, 지대한 관심이 있거나, 앞으로라도 지대한 관심을 가져보실 분

우대사항
- 처음부터 서비스를 만들어 출시해 본 경험이 있으신 분
- 인터렉티브한 디자인 가능하신 분
- 3D 오브젝트 제작 가능하신 분
- 최신 트랜드에 대한 관심이 많으신 분

혜택 및 복지
라이프페스타는 가치 있는 일과 신뢰 하는 동료들과 함께 일과 삶의 조화를 찾아가고 있습니다.
• 업무 성과에 따른 스톡옵션 부여 
• 자율적인 연차 사용 
• 듀얼 모니터 등 직군에 맞는 업무 장비 지원 
• 성장을 위한 도서 구입비 지원 
• 쾌적하고 자유로운 "초"역세권 근무환경''',
    'companyInfo': {
      '기업형태': '중소기업',
      '업종': '에이전시',
      '매출액': '연 매출 2억',
      '사원수': '13명',
      '설립일': '2018년 1월 31일',
      '대표자': '김이나',
    },
    'stastics': {
      'applicant': {
        '지원자 수': 12,
        '모집인원': 0,
      },
      'document': {
        '포트폴리오': 10,
        '기타 문서': 2,
      },
      'career': {
        '신입': 6,
        '1년 미만': 3,
        '1~3년': 2,
        '3~5년': 0,
        '5년 이상': 1,
      },
    },
    'recommand': [
      {
        'id': '1',
        'companyName': '싸이몬',
        'title': 'UX/GUI 디자이너 모집',
        'location_Gu': '강남구',
        'due': '상시',
        'image': 'assets/images/community/recruit/recruit_01.jpg',
      },
      {
        'id': '2',
        'companyName': '이노이',
        'title': 'UI 시각디자이너',
        'location_Gu': '강남구',
        'due': '01.19(금)',
        'image': 'assets/images/community/recruit/recruit_13.jpg',
      },
      {
        'id': '3',
        'companyName': '이그나이트',
        'title': '시각 디자이너 모집',
        'location_Gu': '강남구',
        'due': '01.19(금)',
        'image': 'assets/images/community/recruit/recruit_12.jpg',
      },
      {
        'id': '4',
        'companyName': '캐럿디자인',
        'title': '웹디자이너 신입',
        'location_Gu': '강남구',
        'due': '01.19(금)',
        'image': 'assets/images/community/recruit/recruit_11.jpg',
      },
    ],
  },
];
